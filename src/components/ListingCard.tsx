...
export default function ListingCard({ listing, animated = false, loading = 'lazy' }: ListingCardProps) {
  return (
    <Link
      ...
      className="group block overflow-hidden rounded-lg border border-[var(--line)] bg-[var(--paper-soft)] transition hover:-translate-y-1 hover:border-[var(--ink)] focus-ring"
    >
      ...
      <span className="block p-5">
        <span className="flex items-center justify-between gap-3 text-xs font-bold uppercase">
          <span className="rounded bg-[var(--brand-blue)] px-2 py-1 text-[var(--ink)]">{listing.status}</span>
        ...
        <span className="mt-5 flex items-end justify-between gap-4">
          ...
          <span className="text-sm font-semibold uppercase text-[var(--ink)] underline decoration-[var(--brand-blue)] decoration-4 underline-offset-4">Dettaglio</span>
        </span>
      </span>
    </Link>
  );
